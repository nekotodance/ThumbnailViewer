...
- ファイル削除時設定  
  この機能は、学習時の画像選別時のタグファイルをあわせて削除するために作成しています  
  変更するUIは設けていないので、必要があれば設定ファイル自体をテキストエディタなどで編集してください  
  - delete-textfile   : 0:def 削除しない, 1:削除対象と同じファイル名のtxtファイルがあれば同時に削除する  
...
- H、DELETEキーで選択ファイルを削除[^5]  
- Mキーで選択ファイルをコピー先フォルダ1にムーブ  
- N、F5キーでリストを最新状態に更新  
- ESC、「,」キーでアプリ終了  
...
## 変更履歴
- 0.2.20 画像ファイル削除時に同名のtxtファイルも削除する設定の追加  
- 0.2.19 ムーブ処理を追加、他  
- 0.2.18 リストをファイラーの順序と合わせる（natsortを利用）  
...
[^5]:ファイル削除時の動作について
設定ファイルのdelete-textfileを1にすることにより、画像ファイルと同名のtxtファイルを合わせて削除する機能を0.2.20で追加しています  
